...
function App() {
  ...
  const handleRunReview = async () => {
    ...
    try {
      // Pointing directly to your live Vercel backend deployment
  ...
  return (
    <div className="min-h-screen bg-[#0B1020] text-slate-100 font-sans flex flex-col justify-between overflow-hidden relative selection:bg-indigo-500/30">
      ...
      {/* 1. Ambient Breathing Dynamic Grid Background */}
      <motion.div 
      ...
      {/* 3. Main Split-Pane Workspace Dashboard Container */}
            ...
            {activeTab === 'config' && (
              <motion.div key="config" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 space-y-4">
                ...
                <div className="bg-[#111827] border border-[#2D3748] rounded-xl p-6 max-w-xl space-y-4 shadow-xl">
                  <div className="flex items-center justify-between"><span className="text-xs font-mono text-slate-300">Target Model Provider</span><span className="text-xs font-mono bg-[#0B1020] px-3 py-1 border border-[#2D3748] rounded text-[#06B6D4]">llama3-8b-8192</span></div>
      ...
      {/* 5. Spotlight Search Modal Overlay */}
      <AnimatePresence>
        {isSearchOpen && (
          <motion.div 
